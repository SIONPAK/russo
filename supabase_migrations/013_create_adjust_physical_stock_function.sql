-- 🚀 adjust_physical_stock RPC 함수 생성
-- 물리적 재고 조정 함수 (기존 add_physical_stock과 유사하지만 다른 파라미터)
...
BEGIN
  ...
  IF p_color IS NOT NULL AND p_size IS NOT NULL THEN
    ...
    -- 새로운 물리적 재고 계산
    new_physical_stock := (target_option.option->>'physical_stock')::INTEGER + p_quantity_change;
  ...
  ELSE
    -- 전체 재고 관리인 경우
    new_physical_stock := current_product.stock_quantity + p_quantity_change;
  ...
  INSERT INTO stock_movements (
    ...
    p_product_id,
    CASE 
      WHEN p_quantity_change > 0 THEN 'inbound'
      WHEN p_quantity_change < 0 THEN 'outbound'
      ELSE 'adjustment'
    END,
    p_quantity_change,
    p_color,
