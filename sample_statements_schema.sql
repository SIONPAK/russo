    ...
    -- 반품 명세서 테이블 (return_statements) - 주문과 독립적
    CREATE TABLE IF NOT EXISTS return_statements (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        statement_number VARCHAR(50) UNIQUE NOT NULL, -- 명세서 번호 (RO-YYYYMMDD-XXXX)
        order_id UUID REFERENCES orders(id) ON DELETE SET NULL, -- 주문 연결 (선택사항)
        user_id UUID REFERENCES users(id) ON DELETE CASCADE,
        company_name VARCHAR(255) NOT NULL, -- 업체명 (필수)
    ...
    DROP TRIGGER IF EXISTS update_sample_statements_updated_at ON sample_statements;
    DROP TRIGGER IF EXISTS update_deduction_statements_updated_at ON deduction_statements;
    DROP TRIGGER IF EXISTS update_return_statements_updated_at ON return_statements;
    ...
    BEGIN
        ...
        table_name := CASE 
            ...
            WHEN prefix = 'RO' THEN 'return_statements'
        END;
    ...
    -- 기존 트리거 삭제 후 생성
    DROP TRIGGER IF EXISTS set_sample_statement_number_trigger ON sample_statements;
    ...
    DROP TRIGGER IF EXISTS set_deduction_statement_number_trigger ON deduction_statements;
    ...
    BEGIN
        IF NEW.statement_number IS NULL OR NEW.statement_number = '' THEN
            NEW.statement_number := generate_statement_number('RO');
        END IF;
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;

    DROP TRIGGER IF EXISTS set_return_statement_number_trigger ON return_statements;
